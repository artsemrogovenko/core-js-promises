...
function getPromiseResult(source) {
  const result = source.then(() => 'success').catch(() => 'fail');
  return result;
}
...
function getFirstPromiseResult(promises) {
  return Promise.race(promises).finally((value) => value);
}
...
function getAllOrNothing(promises) {
  return Promise.all(promises).finally((value) => value);
}
...
function getAllResult(promises) {
  const result = Promise.allSettled(promises).then((results) =>
...
function queuePromises(promises) {
  return promises.reduce((chain, promise) => {
    return chain.then((string) => {
      return promise.then((number) => `${string}${number}`);
    });
  }, Promise.resolve(''));
}
